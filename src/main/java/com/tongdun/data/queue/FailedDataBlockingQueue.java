 ...
 */
@Component
public class FailedDataBlockingQueue {

	private Logger logger = LoggerFactory.getLogger(FailedDataBlockingQueue.class);


	// 最大队列数
	 public static final Integer MAX_QUEUE_COUNT = 10000 * 20;
	 // 声明队列
	 private BlockingQueue<Map<String,Object>> failedQueue = new LinkedBlockingQueue<Map<String,Object>>(MAX_QUEUE_COUNT);
	 // 统计已经消费数据数目
	 private AtomicInteger takeCount = new AtomicInteger(0); 
	 // 统计发送失败数据数目
	 private AtomicInteger faildCount = new AtomicInteger(0);

	public FailedDataBlockingQueue() {
		logger.info("创建");
	}
